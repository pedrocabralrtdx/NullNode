...
class AnimationController {
  ...
  private drawTrail(t1: number) {
    for (let i = 0; i < this.trailLength; i++) {
      ...
      this.ctx.fillStyle = i % 3 === 0 ? 'rgba(124, 255, 155, 0.7)' : 'rgba(255, 255, 255, 0.75)'
      this.ctx.lineWidth = sw
...
export function SpiralAnimation({ loop = true, onComplete }: SpiralAnimationProps) {
  ...
  useEffect(() => {
    ...
    animationRef.current = new AnimationController(canvas, ctx, dpr, size, { loop, onComplete })
